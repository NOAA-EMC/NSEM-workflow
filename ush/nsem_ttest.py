...
# storm peak in order to detect 
   ...
   if plotflag:
      ...
      axs[0].set(ylabel=unit, title=label+": p-values = "+"{:.5f}".format(pvalue1)+"; "+"{:.5f}".format(pvalue2))
   ...
   if plotflag:
      ...
      fig, axs = plt.subplots(2)
      axs[0].hist(d1, 20)
      axs[0].set(ylabel="Frequency", xlim=[-(maxdev+1),maxdev+1], title=label+": p-values = "+"{:.5f}".format(pvalue1)+"; "+"{:.5f}".format(pvalue2))
